    ...
    if station_temp:
        if year:
            if int(year) >= 2005:
                ...
                fig = px.scatter(df, x="record_ts",y="value", title=f"Temperature at Station {station_temp}",labels={"record_ts":"Timestamp","value":f"{pl.DataFrame(conn.table("variables").select("name").eq("variable_id",variable).execute().data)[0]} ({conn.table("variables").select("unit").eq("variable_id",variable).execute().data})"})
                st.plotly_chart(fig)
